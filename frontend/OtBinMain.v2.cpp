...
void partyn1_decode(const std::vector<block> inputSet, const block& aesKey, const std::vector <std::vector<block>> okvsTables, std::vector<block>& inputSet2PSI, u64 type_okvs, u64 type_security)
{
...
void partyn_decode(const std::vector<block> inputSet, const std::vector<block> okvsTable, std::vector<block>& inputSet2PSI, u64 type_okvs, u64 type_security)
{
	inputSet2PSI.resize(inputSet.size(), ZeroBlock);
	std::vector<block> hashInputSet(inputSet.size());
...
void partyO1(u64 myIdx, u64 nParties, u64 setSize, u64 type_okvs, u64 type_security)
{
	std::fstream textout;
...
void O1nPSI_Test()
{
	...
	for (u64 pIdx = 0; pIdx < pThrds.size(); ++pIdx)
	{
		pThrds[pIdx] = std::thread([&, pIdx]() {
			...
			partyO1(pIdx, nParties, setSize, GbfOkvs, secMalicious);
			});
	}
